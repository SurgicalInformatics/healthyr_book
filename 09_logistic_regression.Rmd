...
We will use the convenient `summary_factorlist()` function from the `finalfit` package to look for difference across other variables by tumour ulceration.
...
Figure (Figure \@ref(fig:chap09-fig-loess)) shows that age is interesting as the relationship is u-shaped. 
...
Let's use a clever `pivot_longer()` and `facet_wrap()` combination to effiently plot multiple variables against each other without using `ggpairs()`.
...
```{r chap09-fig-contcat, fig.height=3, fig.width=5, message=FALSE, warning=FALSE, fig.cap = "Exploring associations between continuous and catecorical explanatory variables."}
...
In simple language, this is a measure of how much the variance of a particular regression coefficient is increased due to the presence of multicollinearit in the model. 
...
The AIC has been previous described (Section \@ref(chap07-aic)). 
...
<!-- Haven't included diagnostic plots -->
