...
Data is 'tidy' (and therefore also 'long') when *each variable is in its own column*, and *each observation is in its own row*. 
...
And you're right, we won't deny that `pivot_longer()` is one of the most tehnically complicated functions in this book, and it can take a lot of trial and error to get it to work.
