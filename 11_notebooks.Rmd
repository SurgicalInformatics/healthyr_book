...
What is often be under-appreciated is the flexibility with which analyses can be exported or reported.
...
knitr::include_graphics("images/chapter11/1_literate_programming.png", auto_pdf = TRUE)
```
...
* easier to work on smaller screen, i.e., laptop;
...
knitr::include_graphics("images/chapter11/3_help.png", auto_pdf = TRUE)
```
...
Technically, R Markdown is a file, whereas R Notebook a way to work with R Markdown files.
...
In a Notebook, when the file rendered with the `Preview` button, no code is re-run, only that which has already been run in and is present in the document is included in the output. 
...
Figure \@ref(fig:chap11-fig-anatomy) shows the essential elements of a Notebook file and how these translate to the `HTML` preview. 
...
knitr::include_graphics("images/chapter11/2_anatomy_rotated.png", auto_pdf = TRUE)
```
...
Chunk behaviour can be controlled via the `Chunk Cog` on the right of the chunk (Figure \@ref(fig:chap11-fig-anatomy)).
...
Code can be run line line-by-line using `Ctrl+Enter` as you are used to. 
...
knitr::include_graphics("images/chapter11/4_notebook_options_rotated.png", auto_pdf = TRUE)
```
...
PDF document creation require a `LaTeX` distribution to be installed on your computer. 
...
2. `load("melanoma_final.rda")` from the `data_processed/` folder
    + usually quicker, but loads the dataset which was created the last time your ran `00_source_all.R`
