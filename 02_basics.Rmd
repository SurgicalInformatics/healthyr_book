...
The pipe usually sends data to the beginning of function brackets (as most of the functions we use expect data as the first argument). So `mydata %>% lm(dependent~explanatory)` is equivalent to `lm(mydata, dependent~explanatory)`. However, this is not an order that `lm()` expects. `lm()` wants us to specify the variables first (`dependent~explanatory`), and then wants the data these variables are present in. So we have to use the `.` to tell the pipe to send the data to the second argument of `lm()`, not the first.
...
GBD data are publicly available from their website. Table \@ref(tab:chap2-tab-gbd) and Figure \@ref(fig:chap2-fig-gbd) show a very high level version of the project's data with just 3 variables: cause, year, deaths (number of people who die of each cause every year). Later, we will be using a longer dataset with different subgroups and we will show you how to summarise comprehensive datasets yourself.
...
## Variable types and why do we care
...
Number are generally very straightforward to handle and don't cause a lot of trouble. 
...
> FRIENDLY WARNING: What's about to follow is a bit dry. Furthermore, ist is not essential for complete beginners - you might want to continue reading from __Categorical variables - characters__.
Before you leave, take a mental note that sometimes numbers in R have more decimal places than it seems, and that can cause issues when using the double equals operator (`==`).

One minor thing to note about numbers is that sometimes R doesn't print all of the decimal places.
For example, from the `typedata` tibble, we're taking the `measurement` column and sending it to the `mean()` function. 
R then calculates the mean and tells us what it is using 6 decimal places:
...
So overall, since health data is often categrocial and has a reference (comparison) level, then factors are an essential way to work with these data in R.
Nevertheless, the fussiness of factors can sometimes be unhelpful or even frustrating.
It takes experience as an R use to know when is it easiest to keep your variables as characters and when to convert them to factors.
A lot more about factor handling will be covered later in the book.

## Date/time variables

Dates and times sometimes look like character variables - and they are usually printed between double quotes, e.g.:
...
## When data doesn't come in the right way
...
typesdata_faulty %>% 
  mutate(date1 = ymd_hm(date),
         date2 = dmy_hm(date)) %>% 
  select(date, date1, date2)
```
