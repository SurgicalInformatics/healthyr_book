...
The only thing we've changed in (2) is replacing `geom_point()` with `geom_jitter()` - this spread the points out to reduce overplotting.

But what's really exciting is what happens in (3) and (4) of Figure \@ref(fig:chap04-fig-multigeoms).
...
In (4) we included the `aes()` in two places - at the top and inside the `geom_jitter()`.
...
An this is exactly what we see on \@ref(fig:chap04-fig-multigeoms).
