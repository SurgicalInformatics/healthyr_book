...
A useful way to quickly investigate a categorical variables is the`count()` function:
...
With this example `tibble typesdata` that only has three rows, it is pretty obvious that the `id` column is a unique identifier whereas the `group` column is a categorical variable. You can check everything by just eyeballing the tibble using the built in Viewer tab (click on data in the Environment tab).
