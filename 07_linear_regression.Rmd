...
\@ref(fig:chap7-fig-regression) unifies the terms we will use throughout. 
A clear scientific question should define our `explanatory variable of interest` (*x*), which somtimes gets called a predictor. 
...
The explanaotry variables can be either continuous or categorical. 
...
\@ref(fig:chap7-fig-regression) shows a linear regression example. 
Each point is a person and cups of cofee per day is the explanatory variable of interest (*x*) and systolic blood pressure as the dependent variable (*y*). 
...
```{r chap7-fig-regression, echo = FALSE, fig.cap="The anatomy of a regression plot"}
knitr::include_graphics("images/chapter7/1_regression_terms.png")
```
...
\@ref(fig:chap7-fig-residuals) aims to make this easy to understand. 
...
The distance from each observed point to the line is called a *residual* - one of those statstical terms that bring on the sweats. 
...
We can square each residual (to get rid of minuses and penalise far away points) and add them up. 
If this number is as small as possible, the line is fitting as best it can. 
Or in more formal langauge, we want to minimise the sum of squares of residuals. 

```{r chap7-fig-residuals, echo = FALSE, fig.cap="How a regression line is fitted"}
knitr::include_graphics("images/chapter7/2_residuals.png")
```
