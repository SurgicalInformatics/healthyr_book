...
The regression apps and example figures in this chapter have been adapted from https://github.com/mwaskom/ShinyApps and https://github.com/ShinyEd/intro-stats with permission.
...
```{r chap07-fig-diags, echo = FALSE, fig.cap="Regression diagnostics. A - this is what a linear fit should look like. B - this is not approriate, a non-linear model should be used instead."}
...
```{r, fig.height=5, fig.width=7, fig.cap="Scatterplots with linear regression lines: Life expectancy by year in European countries"}
mydata %>%                        
...
```{r fig.height=3, fig.width=4.5, fig.cap="Scatterplot: Life expectancy by year: Turkey and United Kingdom"}
mydata %>% 
...
The slopes make sense - the results of the linear regression say that in the UK, life expectancy increases by `r fit_uk$coefficients[2] %>% round(3)` every year, whereas in Turkey the change is `r fit_turkey$coefficients[3] %>% round(2)` per year.
...
It is important to think about the ranges you can extend your model prediction, and where they just become unrealistic. 
...
We use the `tidy()` function from `library(broom)` to get the variable(s) specific values in a nice tibble:
...
In this particular setting, it will become obvious which model is appropriate.
So we have complete control over the model being fitted, we will use the `predict()` function directly to obtain our fitted line (rather than the `geom_smooth()` as before).  
...
You can check the R-squared using `summary(fit_both1)`.
