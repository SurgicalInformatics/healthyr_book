...
Important to realise: while the `kable()` function from `library(knitr)` is excellent for exporting tables to the three output formats: HTML, PDF, Word; it is limited in customisations that you may need to apply on your table.
For instance, if you use the package `kableExtra()` to customise your tables, you can only export to HTML and PDF. 
Knitting to Work will just not work with advanced `kableExtra` functions in your R Markdown document.
...
A few points of advice on the issue of highly customised tables and different output formats:

* HTML is least fussy to work with and can resize itself and its content automatically. For rapid exploration and prototyping, we recommend knitting to HTML. HTML documents can be attached to emails and viewed using any browser, even with no internet access (as long as it is a self-contained HTML document, which R Markdown exports usually are).
* PDF looks most professional once printed. This is because R Markdown uses LaTeX to typeset PDF documents. LaTeX PDFs are our preferred method of producing printable reports or dissertations, but they come with their own bag of issues. Mainly that LaTeX figures and tables **float** and may therefore appear much later down the document than the original text describing it was.
* Word is used when working with non-R people who need to edit your output.

In this chapter we will demonstrate how tables and figures can be exported to PDF and Word documents.
We will not go into detailed customisisations, because that's much more than a chapter can carry as advanced customisations become format-dependent.
...
We will ask, does a particular characteristic of a colon cancer (e.g., differentiation) predict 5-year survival?
...
knitr::include_graphics("images/chapter12/1_word_knit.png", auto_pdf = TRUE)
```


Knitting this into a Word document results in Figure \@ref(fig:chap12-fig-word)A), which looks pretty decent but the some of the columns need some formatting and the plot needs resized.
...
Move the file to your project folder and refernce it in your `.Rmd` YAML header, as shown below. 
...
The Chunk cog introduced in the previos chapter is a convenient way to change the figure size (it is in the top-right corner of each grey code chunk in an R Markdown document).
...
Knitting Example 2 here gives us Figure \@ref(fig:chap12-fig-word)B).
...
Without changing anything in Example 1 and Knitting it into a PDF, we get \@ref(fig:chap12-fig-pdf)A.
...
```{r chap12-fig-pdf, echo = FALSE, fig.cap="Knitting to Microsoft Word from R Markdown. Before (A) and after (B) adjustment.", out.width="70%"}
knitr::include_graphics("images/chapter12/1_pdf_knit.png", auto_pdf = TRUE)
```
...
The result is shown in Figure \@ref(fig:chap12-fig-pdf)B.
...
knitr::include_graphics("images/chapter12/4_colon_report.png", auto_pdf = TRUE)
```
