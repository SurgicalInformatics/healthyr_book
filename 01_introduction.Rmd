...
Even though R is a language, don't think that after reading this book you should be able to create a new blank file and just start typing in R code like an evil computer genius from a movie. This is not what real world programming looks like.

Firstly, you should be copy-pasting and adapting existing R code examples - whether from this book or later from your own previous scripts.
Re-writing everything from scratch is not efficient.
Yes, you will understand and eventually remember a lot of it.
But to spend time memorising very specific things that can easily be looked up/copied is simply not necessary.
...
In programming, we get infinite re-do's.
...
We see Errors every single day, they are very normal and do not mean that everything is wrong or that you should give up. 
...
Similarly, datasets that you've loaded (or created/modified) appear in the Environment tab.
When you click on a dataset there, it pops up in a really nice and fast viewer. 
...
When you first open up RStudio, you'll also want to install some extra packages to extend base R's functionality.
...
We usually include `library(tidyverse)` at the top of our every script:
