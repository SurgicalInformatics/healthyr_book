...
Furthermore, searching online is also a great way to learn how to do new specific things or to find code examples.
You should copy-paste solutions into your R script to then modify to match what you're trying to do.
We are constantly copying code from online forums or from other's scripts/our own prior scripts.

However, there a many different ways to achieve the same thing in R.
Therefore, sometimes you'll search for help and come across R code that looks nothing like what you've seen in this book. 
That's because the `tidyverse` packages as well as using the pipe (` %>%`) are a relatively new way of doing things, but search engines will often prioritise results that have had more views. 
...
If you're working offline, then RStudio's built in Help tab can help.
