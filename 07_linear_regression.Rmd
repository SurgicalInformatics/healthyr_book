...
We are staying with our example in thinking about whether coffee influences blood pressure. Remember all of these plots use simulated data. 
...
In trying to answer this question using observational data, we have to account for other differences in 

, we usually have to account for other variables which may influence the outcome. 



Figure \@ref(fig:chap07-fig-equation)A shows a scatter plot with fitted lines from a multivariable linear regression model. The plot is taken from the [multiple regression shiny app](https://argoshare.is.ed.ac.uk/multi_regression/). Remember, these data are simulated and are not real. This app will really help you understand different regression models, more on this below. 

Our plot uses 

```{r chap07-fig-dags, echo = FALSE, fig.cap="Causal pathways, effect modification and confounding."}
knitr::include_graphics("images/chapter07/4_dags.pdf")
```

```{r chap07-fig-equation, echo = FALSE, fig.cap="Linking the fitted line, regression equation and R output."}
knitr::include_graphics("images/chapter07/5_equation.pdf")
```
