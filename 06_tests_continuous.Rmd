...
```{r}
africa_data = mydata %>%                 # new object africa_data
...
africa_data %>% 
  gather(key, lifeExp, -country) %>%     # gather original & log vals to same column
  ggplot(aes(x = lifeExp)) +             
  geom_histogram(bins = 15) +            # make histogram
  facet_grid(. ~ key, scales = "free")   # facet by original/log, axes free to vary
```
...
```{r}
africa_plot = mydata %>%                          
  filter(year %in% c(1982, 2007)) %>%           # Filter for 1982 and 2007 only
  filter(continent %in% c("Africa"))            # Filter for Africa only

p1 = africa_plot %>%                            # Plot saved as object p1
  ggplot(aes(x = lifeExp)) + 
  geom_histogram(bins = 15) +                   # bins can be adjusted based on data 
  facet_grid(. ~ year)

p2 = africa_plot %>%                            # Plot saved as object p2
  ggplot(aes(sample = lifeExp)) +               # remember `sample` for Q-Q plot
  geom_qq() + 
  geom_qq_line() + 
  facet_grid(. ~ year)

p3 = africa_plot %>%                            # Plot saved as object p3
  ggplot(aes(x = factor(year), y = lifeExp)) +  # Year is numeric, specify factor directly
  geom_boxplot(aes(fill = factor(year))) +      # Colour in boxplot
  geom_jitter(alpha = 0.4) +                    # Include individual data points
  theme(legend.position = "none")               # Remove legend

library(patchwork)                              # Great package for combining plots
p1 / p2 | p3
...
mydata %>%                          
  filter(year %in% c(1982, 2007)) %>%           # Filter for 1982 and 2007 only
  filter(continent == "Africa") %>%             # Filter for Africa only
  mutate(
    year = factor(year)                         # Change year to a factor
  ) %>% 
...
asia_plot = mydata %>%                          
  filter(continent %in% c("Asia"))              # Filter for Asia only

p1 = asia_plot %>%                              # Plot saved as object p1
  ggplot(aes(x = lifeExp)) + 
  geom_histogram(bins = 15) #+                  # bins can be adjusted based on data 
  #facet_grid(. ~ year)                         # years, not included, so all combined

p2 = asia_plot %>%                              # Plot saved as object p2
  ggplot(aes(sample = lifeExp)) +               # remember `sample` for Q-Q plot
  geom_qq() + 
  geom_qq_line() #+ 
  #facet_grid(. ~ year)                         # years not included, so all combined

p3 = asia_plot %>%                              # Plot saved as object p3
  ggplot(aes(x = factor(year), y = lifeExp)) +  # Year is numeric, specify factor directly
  geom_boxplot(aes(fill = factor(year))) +      # Colour in boxplot
  geom_jitter(alpha = 0.4) +                    # Include individual data points
  theme(legend.position = "none")               # Remove legend
...
# Exercise 2
## Select any 2 years in any continent and perform a *t*-test to determine whether mean life expectancy is significantly different. 
## Remember to plot your data first.

asia_years = mydata %>%                          
  filter(continent %in% c("Asia")) %>%          # filter for Asia only
  filter(year %in% c(1952, 1972))               # filter for 1952 and 1972  

p1 = asia_years %>%                             # Plot saved as object p1
  ggplot(aes(x = lifeExp)) + 
  geom_histogram(bins = 15) +                   # bins can be adjusted based on data 
  facet_grid(. ~ year)                          

p2 = asia_years %>%                             # Plot saved as object p2
  ggplot(aes(sample = lifeExp)) +               # remember `sample` for Q-Q plot
  geom_qq() + 
  geom_qq_line() + 
  facet_grid(. ~ year)                         # years not included, so all combined

p3 = asia_years %>%                              # Plot saved as object p3
  ggplot(aes(x = factor(year), y = lifeExp)) +  # Year is numeric, specify factor directly
  geom_boxplot(aes(fill = factor(year))) +      # Colour in boxplot
  geom_jitter(alpha = 0.4) +                    # Include individual data points
  theme(legend.position = "none")               # Remove legend
...
mydata %>% 
  ...
  group_by(continent) %>%
  do(
    t.test(.$lifeExp, mu = 70) %>% tidy()       # It's somtimes a little awkward in the tidyverse
  )
```

```{r}
# Exercise 4
## Use Kruskal-Wallis to determine if the mean population changed significantly through the 1990s/2000s in individual continents. 
