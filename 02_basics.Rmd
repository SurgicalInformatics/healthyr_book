...
## Object or function, session or environment?

The two fundamental thins to understand about statistical programming are objects and functions. As usual, in this book, we prefer introducing new concepts using specific examples first. And then define things in general terms after examples.
...
mydata %>% 
  knitr::kable(booktabs = TRUE, caption = "Example of a table, including NA values.")

```

Now, regular does not mean it can't have missing values. Missing values are denoted NA, which can stand for either not available or not applicable. In same contexts, these things can have a different meaning, For example:
...
TODO: show how this comes from a Spreadhseet to the Environment, unless that's covered in Chapter 1?

So, our first example of an object that lives in the Environment of your R Session is a tibble called `mydata`. A function that you may want to apply on `mydata` is `mean()`:
